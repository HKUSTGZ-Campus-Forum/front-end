...
## ADR-017：MeetCampus 使用后端权威世界与居民运行时同构

- **日期**：2026-08
- **状态**：已采纳
- **背景**：按页面刷新生成脚本化见闻无法表达持续行走、真实相遇和可追溯的共同活动，也无法验证 20 个居民是否使用同一机制。
- **决策**：所有世界事实由后端事件循环推进；场景提供候选活动，居民基于局部观察、计划、需求、记忆和关系选择候选意图，世界校验后持久化计时行程、双向邀请、活动结果、事件和记忆。前端只按服务器时间插值已持久化路径，并可在单账号内切换居民视角，不自行生成或裁决事实。`is_synthetic` 仅作为审计来源，不进入观察或决策输入。
- **理由**：让移动和相遇在离线时仍持续、所有见闻都有事实来源、AI 失败时仍以同一世界规则运行，并确保内测居民与真人绑定居民不存在两套行为逻辑。
- **相关文档**：`docs/meetcampus/CONTEXT.md`、`docs/plans/2026-08-29-meetcampus-simulation-runtime-design.md`
