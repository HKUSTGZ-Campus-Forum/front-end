...
- **决策**：组队业务页面、API、数据、CI 和发布资产留在外部仓库；主站保留原“组队 / Form a team”名称和图标作为唯一入口，由运行时开关控制并指向同源 `/teamup/`。旧 `/matching` 及其英文、子路径统一永久重定向至对应语言的 `/teamup`，但遗留实现暂不做破坏性删除。接入应用使用 UniKorn JWT 和只读用户身份投影，独立运行于 `/teamup/` 与 `/api/teamup/`，并通过平台审批的精确 SHA 发布。
