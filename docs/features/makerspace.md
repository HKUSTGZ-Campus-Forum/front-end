...
`SpaceCard`, `SpaceActions` and `AuthorLink` are shared by the catalog, work overview and profile collections. Creator names open locale-aware `/users/<id>` links. Profile pages remount when their user ID changes. The current user sees created works (including private drafts) and saved works. Account visibility settings control which tabs visitors see: saved works remain private by default and require explicit opt-in; creator works are shown by default. Visitors see only published works. Hidden sections remain accessible to their owner with an explicit private label. Unpublished saved works are omitted. Removing a favorite updates the collection immediately after server confirmation.
...
## Published sync resource picker

The owner-only `SyncRequestForm.vue` reads `GET /api/makerspace/<slug>/sync/catalog` from the current reviewed public runtime. Authors choose a declared resource/direction and opt into fields; field types and record scope are derived from the declaration, never editable text. The page explains automatic export metadata and client-supplied import protocol fields separately from business fields. Missing resources guide the author to update their own repository and publish a reviewed version. No TeamUp business schema is hardcoded in the platform UI.

Loading, unavailable/malformed/empty declarations and changed-version retries keep submission closed. Reloading or changing resource clears field selection while retaining the usage draft. Submission binds deployment and normalized contract digests; the backend rereads and validates them before creating a pending request. Existing independent approval, one-time credentials and audit controls remain separate. This UI reads schema metadata, not business records, and does not create exchange grants automatically.

## Direct public use

Opening a published hosted work from the catalog or its canonical URL automatically starts the reviewed public runtime after auth restoration. There is no second launch button or preview/session boilerplate for visitors. Work metadata and reactions are available in a compact, expandable About this work panel so the runtime remains the main content. Owners also enter the published version by default; deployment preview controls explicitly switch to a labeled private preview. Closing a preview returns to the published work when available. Independent reviewers retain backend-authorized preview access.

`useMakerRuntime.ts` sequences launches so late responses cannot open the previous work after navigation or an auth change. Errors and slow loading offer retry. Session expiry offers an explicit reopen action without silently reloading a form or discarding unsaved input. Public and private launches retain their existing backend permissions, isolated storage, sandbox and browser binding. Runtime tests cover request bodies, stale responses, expiry and invalid URLs; browser acceptance covers automatic entry and visitor/author states in both locales and viewports.

## External backend handoff

Every owner-side grant now has a visible integration guide with a full gateway URL and a copy-for-AI action. `SyncIntegrationGuide.vue` and `utils/makerspaceSyncHandoff.ts` derive examples from the approved direction and exact field set, using synthetic values and a credential placeholder. The real token is never passed to the guide builder or its clipboard action. The public API origin is used for configured dev connections; private SSR origins and the applicant's external origin are never used as gateway addresses.

The bilingual guide includes first/subsequent export requests, transactional cursor persistence, empty-page polling, deduplication/tombstones, import event retries and version conflicts, quotas and precise error recovery. Revocation clears the token and therefore returns 401 `sync_invalid_credential`; expiry normally returns 403 `sync_not_approved`. Other version/account failures can share these statuses, so the guide does not claim a uniquely identifying expiry/revocation code. Pending, missing-credential and inactive grants explain that examples do not authorize calls. Clipboard rejection exposes selected read-only text for manual copying. No API, data, grant or credential mutation is part of this change.

Verification: `tests/makerspace/sync-handoff.test.ts`, bilingual text checks and desktop/mobile browser checks. The copied grant state is a snapshot; runtime authorization remains server-enforced. Local or synthetic examples do not constitute a successful real external integration.
