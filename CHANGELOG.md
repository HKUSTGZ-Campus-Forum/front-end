...
- **统一登出**：学校 SSO 会话退出本地账号后继续跳转学校统一登出端点。
...
- **课表卡片 section 与课程代码被挤压成两行**：排课工作台课表块此前仅凭"日列宽是否 >150px"的硬编码阈值决定是否把 section 内嵌到课程代码同一行，实际内容放不下时会把代码与 section 挤成两行。改为按每个块的**实际文本宽度**测量（canvas `measureText`，匹配卡片真实字体）并与卡片内容宽度比较：放得下才内嵌，放不下则降级为标题下方的独立图标行；判定抽成可单测的纯函数 `canInlineSection`
