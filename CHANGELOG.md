...
- **排课课程统计改为明确人数**：侧栏课程卡不再显示含义不明的“关注中/正在排课”斜杠数字，改为带可见标签的“购物车”和“方案中”人数；统计分别按课程所在购物车与已保存方案的已验证用户去重计算，并补齐中英文、无障碍说明及窄屏换行。
...
- **排课助手浏览器标签品牌信息**：排课助手学期选择页与工作区补上中英文页面标题，浏览器标签不再回退为全站默认名称；favicon 改为透明外角的科广深蓝底白色标志，在明暗标签栏中均保持清晰。
