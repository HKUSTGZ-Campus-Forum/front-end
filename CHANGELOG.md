...
- **通知打开即已读**：通知页加载成功后自动将已有通知全部设为已读并同步铃铛角标，移除手动已读按钮和未读筛选；同步失败保留列表并提供中英文重试提示。
...
- **首页招新入口**：首页轮播首位新增 NODE 提示词夺旗挑战海报，点击进入本地化招新页面；右下角“安装应用”按钮上方同步增加同尺寸、无 Emoji 的“招新挑战”入口。
...
- **NODE 招新实时排位 Agent 挑战**：新增不出现在主界面导航中的独立双语 `/recruitment` 页面；参与者使用学校账号登录后，可不限总次数地提交不超过 100 个字符的策略，查看每轮行动轨迹与得分，并按个人历史最高分参加实时排行榜；同分时首次达到该分数更早者优先。Agent 不具备 SSH、Shell、生产数据库、主机文件或任意外网权限。
...
- **招新 Prompt 自动通关漏洞**：页面明确提示 Agent 只执行参与者写出的行动步骤，并为过于含糊或超出 Prompt 授权范围的行动补充可读反馈，避免无关短句被误解为完整解题策略。
