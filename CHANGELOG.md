...
- **排课工作台课表卡与课程信息排版精修**：课表卡片中课程名称字号由 `0.75rem` 调大至 `0.82rem`，成为卡片主文本；下方补充信息（section/教室/教师/时间）字号调小至 `0.7rem`，层级更清晰。右侧栏课程卡"· 学分"由 `center` 改为 `baseline` 对齐，与课程代码基线对齐不再偏上；购物车抽屉列表与搜索结果的"课程代码 · 学分"间距由 `8px` 收紧至 `4px`，让学分紧跟代码不再偏右。
