...
- **全局多渲染器桌宠预览**：共享开发环境在所有页面左下角加载固定版本的社区 DeepSeek 女仆鲸 GLB，支持待机与点击动作、折叠和加载失败重试；渲染层可在 Three.js GLB 与 `l2d` Live2D 间切换，移动端与减少动态效果偏好默认收起，正式环境默认关闭并可替换为支持口型参数的自有 Live2D 模型。
