...
describe('TeamUp native host page', () => {
  it('redirects legacy root and deep routes to the canonical MakerSpace URL', () => {
    for (const page of ['pages/teamup/index.vue', 'pages/teamup/[...path].vue']) {
      const source = read(page)
      expect(source).toContain("definePageMeta({ layout: 'keguang' })")
      expect(source).toContain('TEAMUP_SPACE_PATH')
      expect(source).toContain('redirectCode: 308')
      expect(source).toContain('query: route.query, hash: route.hash')
    }
  })
