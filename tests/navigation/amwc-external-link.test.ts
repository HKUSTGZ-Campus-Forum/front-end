...
describe('AMWC external navigation contract', () => {
  ...
  it('uses the same URL for the AMWC home banner external-link branch', () => {
    const carousel = source('../../components/home/CarouselBanner.vue')

    expect(carousel).toContain('import { AMWC_RESULTS_URL } from "~/utils/externalLinks"')
    expect(carousel).toContain('href: AMWC_RESULTS_URL')
    ...
    expect(carousel).not.toContain('href: localePath("/contest")')
  })
