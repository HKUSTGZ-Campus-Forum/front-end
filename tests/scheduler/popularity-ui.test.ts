...
describe('scheduler popularity UI contract', () => {
  ...
  it('refreshes after cart mutations without coupling refresh failures to cart state', () => {
    ...
    const sidePanelStart = dashboard.indexOf('<SchedulerSidePanel')
    const sidePanelEnd = dashboard.indexOf('/>', sidePanelStart)
    const sidePanel = dashboard.slice(sidePanelStart, sidePanelEnd)

    expect(handler.indexOf('await action()')).toBeLessThan(handler.indexOf('await popularity.refresh()'))
    expect(timetable).not.toContain(':show-popularity=')
    expect(timetable).not.toContain(':popularity-by-course=')
    expect(sidePanel).toContain(':show-popularity="popularity.canShowPopularity.value"')
    expect(sidePanel).toContain(':popularity-by-course="popularity.popularityByCourse.value"')
  })

  it('shows privacy-protected course-level counts in course cards only', () => {
    ...
    expect(timetable).not.toContain('getSchedulerCoursePopularity(')
    expect(timetable).not.toContain('block.popularity')
    expect(timetable).not.toContain('SchedulerPopularitySummary')
    expect(courseCard).toContain('props.selected ?? props.course.enabled')
    expect(courseCard).toContain("isSelected && course.selection_policy?.kind === 'module'")
    expect(courseCard).toContain('v-else-if="isSelected" class="course-card__bundles"')
    expect(courseCard).toContain('SchedulerPopularitySummary')
    expect(courseCard).toContain(':counts="popularity"')
    expect(courseCard).toContain(':can-show-history="canShowHistory"')
    ...
    expect(planPreview).not.toContain(':popularity-by-course=')
    expect(planPreview).not.toContain(':show-popularity=')
  })
