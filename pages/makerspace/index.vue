...
<template>
  <main class="maker-page">
    ...
    <template v-else>
      ...
      <div v-else class="maker-grid"><MakerspaceSpaceCard v-for="space in visible" :key="space.id" :space="space" :manage="tab === 'mine'" @updated="updated => { spaces = spaces.map(item => item.id === updated.id ? updated : item) }" /></div>
    </template>
  </main>
</template>
