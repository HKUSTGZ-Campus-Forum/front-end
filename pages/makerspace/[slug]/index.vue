...
<template>
  <main class="maker-page">
    <header class="maker-header"><div><h1>{{ space ? title(space) : t('makerspace.title') }}</h1><p v-if="space?.owner">{{ t('makerspace.by', { name: space.owner.username }) }}</p></div><NuxtLink class="maker-button" :to="getLocalePath('/makerspace')">{{ t('makerspace.back') }}</NuxtLink></header>
